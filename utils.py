...
class YOLOv1Loss(nn.Module):
    ...
    def forward(self, y_true: torch.Tensor, y_pred: torch.Tensor) -> torch.Tensor:
        ...
        x_loss = self.mse_loss(true_boxes[obj_mask][:, 0], y_true[obj_mask][:, 0])
        y_loss = self.mse_loss(true_boxes[obj_mask][:, 1], y_true[obj_mask][:, 1])
        w_loss = self.mse_loss(torch.sqrt(true_boxes[obj_mask][:, 2]), torch.sqrt(y_true[obj_mask][:, 2]))
        h_loss = self.mse_loss(torch.sqrt(true_boxes[obj_mask][:, 3]), torch.sqrt(y_true[obj_mask][:, 3]))
