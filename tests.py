...
m = FastYOLO1.load_from_checkpoint('lightning_logs/version_8185084/checkpoints/epoch=19-step=20.ckpt', map_location='cpu')
#print(m)
image, boxes, labels = next(iter(YOLODataModule(num_workers=3).train_dataloader()))
with torch.inference_mode():
    out: torch.Tensor = m(image)[0]
    # loss = m.training_step((image, boxes, labels), 0)
